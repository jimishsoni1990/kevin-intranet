/* global intranetIsDarkMode, setTimeout */

// Check the color scheme preference and inject the classes if necessary.
if ( document.body.classList.contains( 'intranet-supports-dark-theme' ) ) {
	intranetDarkModeEditorInit();
}
 ...
 * @return {void}
 */
function intranetDarkModeEditorInit( attempt ) {
	...
	if ( intranetIsDarkMode() ) {
		if ( null === container ) {
			// Try again.
			if ( attempt < maxAttempts ) {
				setTimeout(
					function() {
						intranetDarkModeEditorInit( attempt + 1 );
					},
